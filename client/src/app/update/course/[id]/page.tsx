"use client";
import { ICourse2, IFormCourse } from "@/interface/iAll";
import { hideLoading, showLoading } from "@/redux/loadingSlice";
import { showNotif } from "@/redux/notifSlice";
import { notRender } from "@/redux/notRenderSlice";
...
const AddPath = () => {
  const id = GetParamsNumber("id");
  const Router = useRouter();
  const dispatch = useDispatch<AppDispatch>();
  const idAdmin = useSelector((state: RootState) => state.admin.id);

  const [course, setCourse] = useState<ICourse2>();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status == 200) {
        await axios.post(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/api/action-admin`,
          {
            idAdmin,
            action: "Update",
            explanation: `Update course with title course ${course?.title}`,
          }
        );
        return dispatch(
          showNotif({ message: "Update Course Success", type: "success" })
        );
      }
    } catch (error) {
      if (axios.isAxiosError(error)) {
        if (error.status === 403) {
          return dispatch(
            showNotif({ message: "Title Already Exist", type: "info" })
          );
        }
      }
      console.log(error);
      return dispatch(
        showNotif({ message: "Update Course Failed", type: "error" })
      );
    } finally {
    ...
    const getCourseData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/api/course/update/${id}`
        );
        console.log(response);
        if (!response.data[0].payload) {
          setNotFound(true);
          dispatch(showNotif({ message: "Id Is Invalid", type: "error" }));
          return;
        }
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex-1 flex flex-col">
                  {isClient && formData.preview ? (
                    <div
                      className="relative rounded-lg border-2 border-dashed border-blue-600 dark:border-blue-500 overflow-hidden cursor-pointer hover:border-blue-700 dark:hover:border-blue-400 transition-colors h-full min-h-[280px] lg:max-h-[362px] group"
                    ...
                    <div
                      className="rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-blue-600 dark:hover:border-blue-500 cursor-pointer transition-colors h-full min-h-[280px] lg:max-h-[362px] flex items-center justify-center"
                      onClick={handleDivClick}
                    >
                      <input
                        ref={fileInputRef}
