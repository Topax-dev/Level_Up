"use client";
import { IFormCourse, IPath } from "@/interface/iAll";
...
const AddPath = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status == 200) {
        await axios.post(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/api/action-admin`,
          {
            idAdmin,
            action: "Update",
            explanation: `Update path with title path ${path?.name}`,
          }
        );
        return dispatch(
          showNotif({ message: "Update Path Success", type: "success" })
        );
      }
    } catch (error) {
      ...
      console.log(error);
      return dispatch(
        showNotif({ message: "Update Path Failed", type: "error" })
      );
    } finally {
