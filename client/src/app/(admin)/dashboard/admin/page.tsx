...
import { BookOpen, GraduationCap, Route, Users, BarChart3, FileText, TrendingUp } from "lucide-react";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const getDataDashboard = async () => {
      ...
      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/dashboard/admin`)
  ...
  const stats = [
    { label: "Learning Paths", value: `${dataDashboard?.totalPath ?? '-'}`, icon: Route, color: "text-blue-500", bg: "bg-blue-50 dark:bg-blue-500/10" },
    { label: "Active Courses", value: `${dataDashboard?.totalCourse ?? '-'}`, icon: BookOpen, color: "text-emerald-500", bg: "bg-emerald-50 dark:bg-emerald-500/10" },
    { label: "Total Lessons", value: `${dataDashboard?.totalLesson ?? '-'}`, icon: GraduationCap, color: "text-purple-500", bg: "bg-purple-50 dark:bg-purple-500/10" },
    { label: "User", value: `${dataDashboard?.totalUser ?? '-'}`, icon: Users, color: "text-orange-500", bg: "bg-orange-50 dark:bg-orange-500/10" },
     ...(admin.role === 'owner'
    ? [
        {
          label: "Total Admin",
    ...
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-20 py-8">
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="hidden lg:flex items-center gap-4">
                  <div className="text-right">
                    ...
                    <p className="text-sm font-semibold dark:text-white text-gray-900">Today</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="mb-8">
          ...
          <div className={`grid grid-cols-1 sm:grid-cols-2 ${admin.role === 'admin' ? 'lg:grid-cols-4' : 'lg:grid-cols-5'} gap-4`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {menuItems.map((item, index) => {
              ...
              return (
                ...
                >
                  <div className={`absolute opacity-0 group-hover:opacity-5 transition-opacity duration-300`}></div>
                  <div className="relative">
