  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if(axios.isAxiosError(error)) {
        if(error.status === 403) {
          return dispatch(showNotif({ message : 'Email Or Username Already Exist', type : 'info' }))
        }
      }
      console.log(error);
    } finally {
        ...
        <div className="w-11/12 sm:w-2/3 md:w-2/4 lg:w-1/3 p-8 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg">
          ...
          >
            ...
            <div className="text-center">
              <Link
                href="terms"
                className="font-medium text-gray-500 dark:text-gray-400 hover:underline text-sm"
              >
